	...
	const exportAsImage = async () => {
		...
		if (element instanceof HTMLElement) {
			...
			axios({ method: 'post', url: 'https://mocionws.info/', headers, data })
				...
				.catch((error) =>
					console.log(`REQUEST IMAGE SAVE ERROR ====> ${error}`)
				);
	...
	const renderScreen = () => {
		...
		switch (screenACtive) {
			...
			case 3:
				html = (
					<div
						className={`screen screen-two ${screenACtive === 3 && 'active'}`}
					>
			...
			case 4:
				...
				html = (
					<div
						className={`screen screen-four ${screenACtive === 4 && 'active'}`}
					>
						{countdown > 0 && (
							<>
								{/* <h1>¡sonríe!</h1> */}
			...
			case 6:
				html = (
					<div
						className={`screen screen-six ${screenACtive === 6 && 'active'}`}
						role='button'
						aria-hidden='true'
			...
			if (videoElement) {
				...
				if (canvasRef.current) {
					...
					canvasRef.current.appendChild(
						faceapi.createCanvasFromMedia(videoElement)
					);
			...
			{screenACtive < 5 && (
				<>
					<Webcam
						...
						style={{
							display: `${
								screenACtive > 1 && screenACtive < 5 ? 'block' : 'none'
							}`,
						}}
					/>
					<canvas
						ref={canvasRef}
						style={{ display: `${screenACtive === 4 ? 'block' : 'none'}` }}
						className='appcanvas'
					/>
				</>
			)}
